...
setwd("D:/Bill Credit Recon/Kingsville")
list.files(pattern = ".csv")
list.files(pattern = ".xlsx")


#Read input files
Allocation_List<-read_excel("Kingsville Allocation List 20230301.xlsx",sheet="CSEGS Subscriber List")
Host_File<-read_excel("Baltimore Gas & Electric_Kingsville_202302.xlsx",sheet = "_kWh_Summary_Per_List_B")
Total_KWh <-read_excel("Baltimore Gas & Electric_Kingsville_202302.xlsx",sheet = "_Total_Gen_kWh")
HEX_Report<-read_csv("export_2023-05-10T1319.csv")
Attrition_Report<-read_csv("export_2023-05-10T1320.csv")
Previous_month_report <- read_excel("Kingsville Credit Reconciliation Report - February 2023.xlsx",sheet = "February Report")
Accounts_List <- read_csv("VNMSolarProjectExport (4).csv")
Formula<- read_excel("Foumala.xlsx")

#Removing the summary part in the previous report
Previous_month_report<- Previous_month_report[-(1:15),] #enter the row no before the header row
...
Host_File_Filtered <- Host_File[,c('choice_ID','kWh_Subscribed','EQ_kWh_Credited','total_CS_Credit','percentage (as number)','bank_initial_kWh')]

#Removing Duplicates from the filtered Host File w.r.t Choice ID
Host_File_Filtered <- Host_File_Filtered[!duplicated(Host_File_Filtered$choice_ID),]

#Merging Host and Allocation List by Choice ID and saving it as Filtered List 1
Filtered_List_1 <- merge(Host_File_Filtered, Allocation_List_Filtered, by.x = c("choice_ID"),by.y = c("choice_ID_AL"),all  = TRUE)
...
Filtered_List_1$PerDiff <- round((Filtered_List_1$`percentage (as number)`-Filtered_List_1$Percentage_Allocation_List),digits =2)
...
df1 <- Previous_month_report[,c("Choice ID","Utiltiy Final Bank")] #Check column name if error occurs
...
Filtered_List_2 <- Filtered_List_2[!duplicated(Filtered_List_2$choice_ID),]
...
Filtered_List_2 <- merge(Filtered_List_2,df1,by.x = c("choice_ID"),by.y = c("Choice ID"),all.x = TRUE)

#Replacing NA with 0
Filtered_List_2$`Utility Final Bank` <- Filtered_List_2$`Utility Final Bank` %>% replace_na(0)

#Calculating Utility Initial Bank
Filtered_List_2$UIB <- ifelse((Filtered_List_2$bank_initial_kWh>Filtered_List_2$`Utility Final Bank`), (Filtered_List_2$UIB <-Filtered_List_2$bank_initial_kWh),(Filtered_List_2$UIB <- Filtered_List_2$`Utility Final Bank`))
...
#Saving Final Report
xlsx::write.xlsx(as.data.frame(df2),"Final_Report_14July.xlsx",
                 sheetName="Final_Report",
                 col.names=TRUE,append=TRUE,row.names = FALSE,showNA = FALSE)

Host_File_Final <- Host_File[!duplicated(Host_File$choice_ID),]
xlsx::write.xlsx(as.data.frame(Host_File_Final),"Final_Report_14July.xlsx",
                 sheetName="Host_File",
                 col.names=TRUE,append=TRUE,row.names = FALSE,showNA = FALSE)

xlsx::write.xlsx(as.data.frame(Total_KWh),"Final_Report_14July.xlsx",
                 sheetName="Total kWh",
                 col.names=TRUE,append=TRUE,row.names = FALSE,showNA = FALSE)

xlsx::write.xlsx(as.data.frame(Allocation_List_Filtered),"Final_Report_14July.xlsx",
                 sheetName="Allocation_List",
                 col.names=TRUE,append=TRUE,row.names = FALSE,showNA = FALSE)

xlsx::write.xlsx(as.data.frame(Formula),"Final_Report_14July.xlsx",
                 sheetName="Formula",
                 col.names=TRUE,append=TRUE,row.names = FALSE,showNA = FALSE)
